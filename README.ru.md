...
Прогон - это четыре задачи: забрать сырье за дату, проверить, что оно приехало,
собрать витрины за гейтом, объявить опубликованное.
...
| Конверсия выше порога "заказы > трафик × 0,95" | Флаг |
...
И код, в том порядке, в каком через него идут данные:

| Каталог | Что там лежит |
|---|---|
| `generator/` | Делает синтетику и закладывает пять дефектов |
| `db/` | Схема сырого слоя, единственный DDL, написанный руками |
| `dbt/` | 14 моделей в трех слоях, 93 теста, макросы, снимок |
| `airflow/` | DAG и колбэк алерта - четыре задачи, больше ничего |
| `checks/` | Сверка, намеренно на pandas, а не на SQL |
| `scenarios/` | Три демонстрации, каждая умеет провалиться |
| `docs/` | Собирает `DICTIONARY.md` из описаний в yml |
