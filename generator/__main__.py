...
def cmd_day(config: Config, target: date) -> int:
    """Пересобрать одну партицию - то, чем на этапе 4 станет пересчет.
...
def cmd_measure_returns(config: Config) -> int:
    """Замер, от которого зависит размер окна пересчета на этапе 4.

    Число берется из данных, а не из констант генератора. Так и должно быть:
    на этапе 4 окно придется обосновывать наблюдением, а не тем, что кто-то
    так задумал.
    """
    query = """
        WITH delays AS (
    ...
    print()
    print("  Это вход для этапа 4: окно пересчета выбирается по этим числам,")
    print("  а тест на возврат за пределами окна ловит тех, кто в него не влез.")
    return 0
