...
--
-- Период, а не весь горизонт: сверка на этапе 4 будет идти в каждом ежедневном
-- прогоне, и гонять по трем с половиной миллионам строк каждый раз незачем.
-- Размер окна задается переменной reconcile_days.
