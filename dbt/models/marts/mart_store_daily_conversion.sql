...
-- один набор.
--
-- Замер, который стоит знать до этапа 3: синтетика строит трафик от
-- офлайн-заказов вместе с отмененными, поэтому измеренная конверсия
-- систематически ниже заложенной примерно на долю отмен, около 3%. На
-- срабатывание порога "чеки больше трафика на 0,95" это не влияет никак, но
-- тест на диапазон надо писать сразу с этим смещением, а не выяснять его по
-- красному тесту.
--
