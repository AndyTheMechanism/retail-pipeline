...
# up в зависимостях не для симметрии с остальными целями. Без него на
# остановленном контейнере команда падает с "can only create exec sessions on
# running containers" - это сообщение про внутренности движка, а не про то,
# что база не поднята. Ловится оно после первой же перезагрузки машины.
psql: up ## Открыть psql в контейнере, ставить клиент на хост не нужно
